...
public class NURBSCurvatureUtility {
	...
	private static Logger logger = Logger.getLogger(NURBSCurvatureUtility.class
			.getName());
	 ...
	 */
	public static CurvatureInfo curvatureAndDirections(NURBSSurface ns,
			double[] point) {
		double u = point[0];
		double v = point[1];
		...
		NURBSAlgorithm.SurfaceDerivatives(nl, p, U, ml, q, V,
				ns.getControlMesh(), u, v, 4, SKL1);
		...
		// lambda
		Double lambda = (a11 + a22) / 2
				+ Math.sqrt((a11 - a22) * (a11 - a22) + 4 * a12 * a21) / 2;
